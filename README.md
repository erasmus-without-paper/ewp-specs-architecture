 ...
 * Servers and clients MUST be implemented in a way which will allow us to
   add support for HTTP content negotiation in the future. This means that,
   for example, if the content type requested by the client in his request's
   `Accept` header doesn't match the content type being generated by the
   server, then the server SHOULD respond with the **HTTP 406** errors status
   (preferably with a proper developer message).

   Similarly, servers SHOULD produce a proper `Content-Type` header, matching
   the type of the content they serve (most often, this will be `text/xml` or
   `application/xml`). Clients are NOT REQUIRED to accept responses with
